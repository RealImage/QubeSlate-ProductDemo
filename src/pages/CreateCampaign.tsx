...
const CreateCampaign = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    targetGroups: [{
      ...
      includedScreens: [],
      excludedScreens: [],
      screenSelectionMethod: "screenId",
      screenIds: "",
      theatreIds: "",
      selectedCampaign: "",
      selectedTG: "",
  ...
  const [activeSelectionMethod, setActiveSelectionMethod] = useState("screenId")
  ...
  const addTargetGroup = () => {
    const newTG = {
      id: `tg-${formData.targetGroups.length + 1}`,
      name: "",
      validityStartDate: "",
      validityEndDate: "",
      includedScreens: [],
    ...
    setFormData(prev => ({
      ...prev,
      targetGroups: [...prev.targetGroups, newTG]
    }))
  }
  ...
  const updateTargetGroup = (index: number, field: string, value: any) => {
    setFormData(prev => ({
      ...prev,
      targetGroups: prev.targetGroups.map((tg, i) => 
        i === index ? { ...tg, [field]: value } : tg
      )
    }))
  }

  const addScreenToTG = (tgIndex: number, screen: any, type: 'included' | 'excluded') => {
    const field = type === 'included' ? 'includedScreens' : 'excludedScreens'
    updateTargetGroup(tgIndex, field, [...formData.targetGroups[tgIndex][field], screen])
  }

  const removeScreenFromTG = (tgIndex: number, screenIndex: number, type: 'included' | 'excluded') => {
    const field = type === 'included' ? 'includedScreens' : 'excludedScreens'
    const screens = formData.targetGroups[tgIndex][field]
    updateTargetGroup(tgIndex, field, screens.filter((_, i) => i !== screenIndex))
  }

  const getScreenCount = (tg: any) => {
    return {
      included: tg.includedScreens.length,
      excluded: tg.excludedScreens.length,
      total: tg.includedScreens.length - tg.excludedScreens.length
    }
  }
        ...
        return (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-muted-foreground">
                Define target groups by selecting included and excluded screens. Each target group represents a specific set of screens where your campaign will be displayed.
              </p>
            </div>

            {/* Target Groups List */}
            <div className="space-y-6">
              {formData.targetGroups.map((tg, tgIndex) => {
                const screenCount = getScreenCount(tg)
                return (
                  <Card key={tg.id} className="p-6">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-accent-brand rounded-full flex items-center justify-center">
                          <Target className="w-4 h-4 text-accent-brand-foreground" />
                        </div>
                        <div>
                          <h3 className="text-lg font-semibold text-foreground">
                            Target Group {tgIndex + 1}
                          </h3>
                          <div className="text-sm text-muted-foreground">
                            {screenCount.total > 0 ? `${screenCount.total} screens selected` : 'No screens selected'}
                          </div>
                        </div>
                      </div>
                      {formData.targetGroups.length > 1 && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => removeTargetGroup(tgIndex)}
                        >
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      )}
                    ...
                    {/* TG Basic Information */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div>
                        <Label>TG Name *</Label>
                        <Input
                          placeholder="Enter unique TG name"
                          value={tg.name}
                          onChange={(e) => updateTargetGroup(tgIndex, 'name', e.target.value)}
                          className="mt-1"
                        />
                      </div>
                      <div>
                        <Label>TG Validity Start Date</Label>
                        <Input
                          type="date"
                          value={tg.validityStartDate}
                          onChange={(e) => updateTargetGroup(tgIndex, 'validityStartDate', e.target.value)}
                          className="mt-1"
                        />
                      </div>
                      <div>
                        <Label>TG Validity End Date</Label>
                        <Input
                          type="date"
                          value={tg.validityEndDate}
                          onChange={(e) => updateTargetGroup(tgIndex, 'validityEndDate', e.target.value)}
                          className="mt-1"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <h4 className="text-base font-medium text-foreground">Screen Selection</h4>
                      
                      <Tabs 
                        value={tg.screenSelectionMethod} 
                        onValueChange={(value) => updateTargetGroup(tgIndex, 'screenSelectionMethod', value)}
                        className="w-full"
                      >
                        <TabsList className="grid w-full grid-cols-4">
                          <TabsTrigger value="screenId" className="text-xs">Screen ID</TabsTrigger>
                          <TabsTrigger value="theatreId" className="text-xs">Theatre ID</TabsTrigger>
                          <TabsTrigger value="previousCampaign" className="text-xs">Previous Campaign</TabsTrigger>
                          <TabsTrigger value="location" className="text-xs">Location</TabsTrigger>
                        </TabsList>

                        <TabsContent value="screenId" className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <Label>Copy-Paste Screen IDs</Label>
                              <Textarea
                                placeholder="Enter Screen IDs (comma or line separated)&#10;e.g. SCR001, SCR002&#10;SCR003"
                                value={tg.screenIds}
                                onChange={(e) => updateTargetGroup(tgIndex, 'screenIds', e.target.value)}
                                className="mt-1"
                                rows={4}
                              />
                            </div>
                        ...
                        <TabsContent value="theatreId" className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <Label>Copy-Paste Theatre IDs</Label>
                              <Textarea
                                placeholder="Enter Theatre IDs (comma or line separated)&#10;e.g. THR001, THR002&#10;THR003"
                                value={tg.theatreIds}
                                onChange={(e) => updateTargetGroup(tgIndex, 'theatreIds', e.target.value)}
                                className="mt-1"
                                rows={4}
                              />
                            </div>
                        ...
                        <TabsContent value="previousCampaign" className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <Label>Select Campaign</Label>
                              <Select 
                                value={tg.selectedCampaign} 
                                onValueChange={(value) => updateTargetGroup(tgIndex, 'selectedCampaign', value)}
                              >
                                <SelectTrigger className="mt-1">
                                  <SelectValue placeholder="Choose a previous campaign" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="campaign-1">Summer Movie Campaign</SelectItem>
                                  <SelectItem value="campaign-2">Holiday Special Campaign</SelectItem>
                                  <SelectItem value="campaign-3">Action Movie Promo</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                            <div>
                              ...
                              <Select 
                                value={tg.selectedTG} 
                                onValueChange={(value) => updateTargetGroup(tgIndex, 'selectedTG', value)}
                              >
                                <SelectTrigger className="mt-1">
                                  <SelectValue placeholder="Choose target group" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="tg-1">Metro Cities TG</SelectItem>
                                  <SelectItem value="tg-2">Tier 2 Cities TG</SelectItem>
                                  <SelectItem value="tg-3">Premium Screens TG</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                          </div>
                        ...
                        <TabsContent value="location" className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <Select 
                                value={tg.selectedLocationType} 
                                onValueChange={(value) => updateTargetGroup(tgIndex, 'selectedLocationType', value)}
                              >
                                <SelectTrigger className="mt-1">
                            ...
                            <div>
                              <Label>Search & Select Locations</Label>
                              <div className="mt-1">
                                <Input
                                ...
                                />
                                <div className="text-sm text-muted-foreground">
                                  Selected: Mumbai, Delhi, Bangalore
                                </div>
                              </div>
                            </div>
                          </div>
                    ...
                    {/* Included/Excluded Screens */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* Included Screens */}
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <Label className="text-base font-medium text-foreground">
                            Included Screens ({screenCount.included})
                          </Label>
                          <Button variant="outline" size="sm">
                            <Plus className="w-4 h-4 mr-1" />
                            Add
                        ...
                        <div className="border border-border rounded-lg p-4 min-h-[120px] max-h-[200px] overflow-y-auto">
                          {tg.includedScreens.length === 0 ? (
                            <p className="text-sm text-muted-foreground text-center py-8">
                              No screens included yet
                            </p>
                          ) : (
                            <div className="space-y-2">
                              {tg.includedScreens.map((screen, index) => (
                                <div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                                  <span className="text-sm">{screen.id || `Screen ${index + 1}`}</span>
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => removeScreenFromTG(tgIndex, index, 'included')}
                                  >
                                    <X className="w-3 h-3" />
                                  </Button>
                                </div>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <Label className="text-base font-medium text-foreground">
                            Excluded Screens ({screenCount.excluded})
                          </Label>
                          <Button variant="outline" size="sm">
                            <Plus className="w-4 h-4 mr-1" />
                            Add
                        ...
                        <div className="border border-border rounded-lg p-4 min-h-[120px] max-h-[200px] overflow-y-auto">
                          {tg.excludedScreens.length === 0 ? (
                            <p className="text-sm text-muted-foreground text-center py-8">
                              No screens excluded
                            </p>
                          ) : (
                            <div className="space-y-2">
                              {tg.excludedScreens.map((screen, index) => (
                                <div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                                  <span className="text-sm">{screen.id || `Screen ${index + 1}`}</span>
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => removeScreenFromTG(tgIndex, index, 'excluded')}
                                  >
                                    <X className="w-3 h-3" />
                                  </Button>
                                </div>
                    ...
                    <div className="mt-6 p-4 bg-muted rounded-lg">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">Total Screens in this TG:</span>
                        <Badge variant={screenCount.total > 0 ? "default" : "secondary"}>
                          {screenCount.total} screens
                        </Badge>
                      </div>
                    </div>
                  </Card>
              ...
              <Card className="p-6 border-dashed">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-medium text-foreground mb-2">Add Another Target Group</h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    Create additional target groups to further segment your campaign reach.
                  </p>
                  <Button onClick={addTargetGroup} variant="outline">
                    <Plus className="w-4 h-4 mr-2" />
                    Add Target Group
                  </Button>
                </div>
